...
🔴 **KHÔNG được:**
- Bỏ qua bất kỳ entity hiện có nào
- Thay đổi structure mà không giải thích
- Tạo code không complete
...
## VII. EXAMPLE: Sample Model Update

**Ví dụ về Car Model hoàn chỉnh:**
...
namespace RentalCarManagement.Models
{
    ...
    public class Car
    {
        ...
        public virtual ICollection<CarDocument> Documents { get; set; }
        ...
        [NotMapped]
        public int TotalRentals => Rentals?.Count(r => r.Status == RentalStatus.Completed) ?? 0;
    }
}
```
...
### Bước 1: Share toàn bộ code hiện tại
- Upload tất cả các file Models
- Upload AppDbContext
- Upload Program.cs (nếu có)

### Bước 2: Yêu cầu AI
"Dựa trên prompt chi tiết ở trên và code hiện tại của tôi, hãy:
1. Phân tích và đánh giá cấu trúc hiện tại
2. Đề xuất các cải tiến cụ thể
3. Tạo code hoàn chỉnh cho từng phần cần update
4. Giải thích lý do cho mỗi thay đổi"

### Bước 3: Review và apply changes
- Xem xét từng đề xuất
- Apply từng phần một
...
- [ ] Tất cả Models có đầy đủ properties
- [ ] Relationships được define rõ ràng
- [ ] Data annotations đầy đủ
- [ ] DTOs được tạo cho tất cả operations
- [ ] Repository pattern implemented
- [ ] Service layer với business logic
- [ ] Controllers với proper endpoints
- [ ] Authentication & Authorization setup
